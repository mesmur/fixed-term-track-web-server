...
func (c *FixedTermController) CreateFixedTerm(ctx *gin.Context) {
	...
	err := c.fixedTermService.Create(&fixedTerm)
	...
	ctx.JSON(http.StatusCreated, fixedTerm)
}
...
func (c *FixedTermController) UpdateFixedTerm(ctx *gin.Context) {
	...
	err := c.fixedTermService.Update(&fixedTerm)
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func (c *FixedTermController) CreateReturn(ctx *gin.Context) {
	...
	err = c.fixedTermService.CreateReturn(&fixedTermReturn)
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}

	ctx.JSON(http.StatusCreated, fixedTermReturn)
}
